...
This integration supports only the Forge 1.20.1 release
`2.6.5-forge+mc1.20.1`. Existing configs receive the two experimental fields
...
The 1.20.1 Forge branch exposes a Forge-native event. Subscribe on the Forge event bus:
...
This branch uses Forge's `@SubscribeEvent` and `event.getRegistry()`, not Fabric's `EVENT.register(...)`.
