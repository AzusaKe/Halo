...
初次提取的 Core：`AzusaKe/HaloCore` 的 `v1.3.0`，提交 `a12100e48e0e71d07e96c7b7dc6915570ae48861`。最终验收基准见下文 1.3.1 记录。
...
2026-09-12，用户在修复权杖滚动条和缺失定义提醒后，继续进行了约 10 分钟多人游戏测试，未见异常，
确认重构成功，并指定当前代码作为 **1.3.1** 版本基准。Halo 与 HaloCore 均以 `v1.3.1` 标签记录该基准，
Halo 通过提交的 core gitlink 锁定对应源码；成品版本为 `1.3.1+adapter.1`。

- 权杖列表的滚动条完整位于鼠标命中范围内；隔离客户端已验证向下和向上拖动。
- 客户端缺少定义时恢复中英文聊天提示，包含定义 ID、资源包安装和联系管理员说明。
  core 负责每 30 秒最多一次的提醒，适配层负责显示；保留佩戴关系，补齐资源后恢复绘制。
- 隔离专用服向客户端同步缺失定义时，实际聊天提示正常，两次提醒间隔 30 秒。
- 固定时钟回归覆盖首次提醒、跨实体限流、定义恢复、实体卸载以及换世界/断连重置。
- 自动检查共 325 项 JUnit：core 242 项通过，Halo 82 项通过、1 项外部 YSM 签名检查跳过，无失败。
  core 独立性、最终 JAR 内容和旧锚点 API v2 调用方检查通过。
- 开发专用服使用独立的 `runServer/1.20.1-fabric/`，已验证启动、控制台操作和正常保存退出。

本次重构工作到此验收完成；后续发现的具体问题作为维护修复处理。
