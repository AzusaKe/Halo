...
Halo 2.0.0 正式版新增 `mesh` 图元，JSON schema 为 **1.1.0**。旧图元和旧定义继续兼容。
先在资源包中放置文件，再从定义引用：
...
4. 每个面角点必须有 UV。支持 `v/vt`、`v/vt/vn`、正负索引和独立 UV 接缝；有效 `vn` 会归一化并保留硬边。缺失或为零的法线按面生成。
...
| `effects[].texture` | 必填 | 灰度遮罩 PNG；可使用与基础 PNG 不同的任意正分辨率及长宽比 |
...
基础纹理与遮罩的尺寸和长宽比相互独立，例如 `128×128` 基础纹理可以搭配 `512×256` 遮罩。
两张图共用完整的归一化 UV 域，各自按原生分辨率读取；资源作者应让需要重合的图案在各自图片中的归一化位置一致。
Halo 不降低任一图片的分辨率、不平均，也不生成或重复上传缩放贴图，因此高分辨率遮罩的细节完整保留。
...
亮度继续受 group 的 `glowing`、`animation.glow` 及继承规则控制。`glowing:false` 在 lightmap 之外还使用 Minecraft 实体管线的双方向表面光照，法线朝向不同的面会有明暗差异。`glowing:true` 不参与该计算，继续保持原有全亮、各角度亮度一致的效果。
渐变透明使用深度测试和普通透明排序；相交网格及与原版水、玻璃交叠仍可能存在常规透明排序局限。

无光影时 mesh 仍在实体缓冲区完成后提交，避免不写深度的透明 mesh 被随后刷出的实体覆盖。开启 Iris 时，不透明且非自发光的 mesh 会在光影包消费 solid G-buffer 前提交；需要混合透明的 mesh 与自发光 mesh 保持既有晚期提交。
...
开启 Iris 时，自发光仍从原有粒子程序创建扁平材质；非自发光另从普通实体漫反射程序创建带法线的材质变体，先处理基础贴图的遮罩 alpha，
...
具体颜色、方向光、雾、bloom 等效果由光影包决定；实体格式会向 Iris 提供法线及其扩展顶点数据。带法线的缓存按三角形角点展开，保证 Iris 的切线生成不跨越无关三角形。若片元程序通过同一 world-space 位置的屏幕导数重建面法线，Halo 会以通用 GLSL 数据流检测恢复上传的平滑法线；检测不包含特定光影包名称、输出目标或局部变量标识，空间不明确时不改写。本版仍不新增 Halo 自己的 LabPBR 材质映射或独立 mesh 投影通道。
...
文件缺失或损坏只跳过相关 mesh；不同分辨率或长宽比本身不会导致 mesh 被跳过。
