...
```sh
# 发布前，先推送 core 的锁定提交，再提交 Halo 中的指针与适配修改：
./gradlew build -Prelease=true
```

发布检查要求两个工作树干净、core HEAD 等于 Halo **已提交**的 gitlink，且该精确提交能从 core 的 origin 获取。
校验在临时空仓库中按 SHA 获取提交，结束后清理；不会更新 core 工作树或跟随任何分支。
这既允许没有版本标签的维护提交，也允许锁定落后于远端分支最新位置的历史提交；仅有本地对象或缓存的远端引用不算验证通过。
功能版本标签仍用于标识功能发布，但不再作为源码锁定或发布检查的额外条件。文档、忽略规则等维护变更可保留功能版本，
不要移动已发布的标签。功能变化时仍先发布新的 core 功能版本，再更新 Halo 的指针。
开发构建不自动提交、切换分支、追踪最新提交或下载另一版 core。
