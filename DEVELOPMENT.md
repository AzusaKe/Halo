...
本分支是 Halo 的 `26.3-fabric` 迁移适配器，以 `26.2-fabric` 与固定 HaloCore 2.4.1 提交为功能基准。目标只声明 Minecraft 26.3，实际游戏验收状态见迁移记录。已验收的 mesh 正式版为 2.1.0，早期重构基准为双方的 `v1.3.1`。开始任务时应检查实际分支。所有 `*-flash` 分支保持冻结。

**常规流程：先定义功能的数据与规则 → 在 core 实现和验证 → 在主线实现适配器并联调 → 发布确定的 core 提交 → 提交主线的适配器及 core 指针 → 按需更新其他版本的指针和适配器 → 分别验收、推送和发布。** 不要求所有游戏版本同时跟进。

文中的 `entity-opacity`（根据实体状态调整光环透明度）仍是完整流程示例。2.0.0 的原生 mesh 功能与正式发布验收见 [mesh 验收记录](docs/mesh-verification.md)；以下 1.4.0 等版本发布命令仍是流程示例。本适配器的迁移边界和验证结果见 [26.3 Fabric 迁移记录](docs/26.3-fabric-migration.md)。本次排除 YSM 和 Connector。下文旧版本发布示例不代表本分支已经验收。
...
26.3 适配器使用 Java 25、Gradle wrapper 9.6.0 和固定 Loom 1.17.21（官方名称，无 remap）；core 源码仍以 Java 17 为契约基线。始终使用仓库的 wrapper。以下命令从 Halo 根目录执行：
...
| `.\gradlew.bat runServer --console=plain` | `runServer/26.3-fabric/` |
...
git clone --branch 26.3-fabric --recurse-submodules https://github.com/AzusaKe/Halo.git ../Halo-1.21.1-fabric
