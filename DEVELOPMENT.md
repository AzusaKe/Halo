...
Modrinth / CurseForge 的集中发布入口、凭据配置、只读预览和失败补发见 [平台发布自动化](docs/release-automation.md)。目标标签 CI 成功后，agent 必须先修改并核对 Release 最终正文，再通过默认分支的 `scripts/release/dispatch.py --notes-file <定稿文件> --publish` 触发上传，绑定正文 SHA-256；CI 完成不会抢先上传。平台上传或 CI 成功不能替代游戏验收。
