...
- [ ] **LabPBR Materials**: This frozen flash branch does not support LabPBR companion textures. Iris may discover same-basename `_n.png`/`_s.png`, but billboard and ring primitives use Minecraft's basic position/texture/color shader without normals or entity-material samplers; `glowing: false` only applies ambient brightness. In-game A/B testing confirmed that the companion maps have no material effect.
