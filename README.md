...
![MC Version](https://img.shields.io/badge/Minecraft-26.1--26.1.2-green.svg)
...
Currently available for Minecraft 26.1 ~ 26.1.2 with Fabric (NeoForge via [Sinytra Connector](https://modrinth.com/mod/connector) works — core features such as rendering and persistence have been tested).
...
- [x] **LabPBR Compatibility**: In world rendering, `billboard` and `ring` primitives can use Iris-collected LabPBR companion textures. Set the containing group to `glowing: false`, then place same-basename `_n.png` and/or `_s.png` maps beside the base texture. In-game testing confirmed the material path; the active shader pack must support LabPBR in its translucent entity pass, and effects such as parallax/POM remain shader-dependent.
...
1. Install [Fabric Loader](https://fabricmc.net/use/) 0.19.3+ for Minecraft 26.1 ~ 26.1.2.
...
1. Install NeoForge for Minecraft 26.1 ~ 26.1.2
...
Halo's Yes Steve Model (YSM) integration is optional and version-pinned. This branch supports exactly **YSM `2.6.5-neoforge+mc26.1`**. If YSM is missing, disabled, or a different version is installed, Halo safely uses its standard entity anchor instead. YSM is only needed on clients that render YSM models; the server does not need YSM for Halo synchronization.
...
The compiled JAR file will be at `build/libs/halo-26.1.2-fabric-1.2.1.jar`.
...
- **Development Environment**: Minecraft 26.1.2 + Fabric Loader 0.19.3+
