...
- 复用已有 core 的透明排序与展开索引入口，保留实际视深顺序和镜像绕序；动态索引仅在修订变化时上传，复用原生暂存内存。
- 原生／GUI 与 Iris 缓冲分离，保留资源／光影重载释放、上传失败记忆及同帧 CPU 回退。core 保持 2.4.1 / `295bc57`，没有新增生产代码或公共契约改动。
- 123 项平台测试与 326 项 core 测试通过；用户确认候选测试通过并授权发布。未进行量化 FPS 对照，不宣称具体提升倍数；验证范围见[迁移记录](docs/26.1-fabric-migration.md)。

正式版本：`2.4.1+adapter.2`；平台标签为 `v2.4.1-fabric-26.1-adapter.2`。
