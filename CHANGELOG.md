...
- 按实际投影重叠深度修正半透明三角面排序；常驻 GPU 和 CPU 回退共用完整视图，覆盖透视、正交及镜像。
- 128 个视图的自动检查中错误排序采样从 2,528 次降至 0；构建、合包及旧接口兼容检查通过。
- 用户确认原报告场景已修复；不扩大为本平台全部渲染组合已实测。交错、跨 mesh、OIT 和渲染管线改造不在本次范围。
- 游戏性能与未明确测试的 Iris/LabPBR 组合不声明验收通过。详见 [修复验证记录](docs/mesh-transparency-fix.md)。

## 2.4.1+adapter.2 — Minecraft 26.3 Fabric — 存档兼容修复 — 2026-09-20
...
- 新增多记录、空列表、读取后修改并保存及 flash Codec 回退读取的兼容性回归测试；HaloCore 保持 2.4.1 / `295bc579ab8d63e6dba5642ebb9e889516e90105`。详情见 [迁移记录](docs/26.3-fabric-migration.md)。

正式版本：`2.4.1+adapter.2`；平台标签为 `v2.4.1-fabric-26.3-adapter.2`。

## 2.4.1+adapter.1 — Minecraft 26.3 Fabric — 2026-09-19

- 从 26.2 adapter.2 迁移到 RenderPearl，适配管线、缓冲、RenderPass、GLSL、鼠标输入、权杖和 EMF 3.3.6 接口；保留本体身份筛选及预览物理。
- 在进入世界／预览 RenderPass 前准备 GPU 资源，绘制阶段消费同帧快照；保留四元数、驻留几何、Iris 顶点格式缓存和透明排序，为延迟提交的每个透明实例分配独立可复用索引槽。
- 修复 Iris 编译管线重排材质绑定导致的 mask 失效；按实际布局绑定遮罩与材质参数，保留 LabPBR 和通用平滑法线匹配。
- 增加默认关闭的 JFR 帧间隔／几何与索引上传计数，便于高面数压测。
- 132 项适配层测试与 326 项 core 测试通过，无失败或跳过；用户确认显示、操作、性能、平滑法线、LabPBR 和遮罩修复正常，完成专有环境网络测试后确认当前版本可正式发布。
- core 保持 2.4.1 / `295bc579ab8d63e6dba5642ebb9e889516e90105`；schema、存档、网络与 API v2 不变。验收范围与未单独执行项见 [26.3 迁移记录](docs/26.3-fabric-migration.md)。
...
- 128 项适配层测试与联合构建通过，新增披风／盔甲抢占、预览及阴影回归测试；用户在原问题实例确认修复。详情与未重测范围见 [EMF 披风修复记录](docs/26.2-fabric-emf-cape-fix.md)。
- HaloCore 保持 2.4.1 / `295bc579ab8d63e6dba5642ebb9e889516e90105`；schema、存档、网络与 API v2 不变。
