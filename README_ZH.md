...
- [x] **LabPBR 兼容**：在世界渲染中，`billboard`、`ring` 和 OBJ `mesh` 三种图元均可使用由 Iris 收集的 LabPBR 附加贴图。将图元所在组设为 `glowing: false`，再在基础贴图同目录放置同名的 `_n.png` 和／或 `_s.png`。所用光影仍须在实体渲染阶段支持 LabPBR；视差／POM 等效果是否生效取决于光影实现。
