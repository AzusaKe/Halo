本分支迁移至 **Minecraft 26.2 Fabric**，版本为 **2.4.2+adapter.1**，锁定 HaloCore **2.4.2**。玩家仍只安装一个 Halo JAR。flash 分支保持冻结；原版 OpenGL/Vulkan、Iris 与 EMF/ETF 已实测，具体版本与验证边界见 [26.2 迁移记录](docs/26.2-fabric-migration.md)。不包含 YSM 与 Vulkan 光影。
...
编译好的 JAR 文件位于 `build/libs/`；当前源码版本生成 `halo-26.2-fabric-2.4.2+adapter.1.jar`。工作树有改动或 core 未正确锁定时，构建名称带 `.dev` 后缀。正式发布需要完成双仓库锁定和验收。
