...
- [ ] **LabPBR 材质**：此冻结的 flash 分支不支持 LabPBR 附加贴图。Iris 可能发现同名的 `_n.png`／`_s.png`，但 billboard 和 ring 图元使用 Minecraft 的基础位置／纹理／颜色 shader，不提供法线或实体材质采样器；`glowing: false` 只会应用环境亮度。游戏内 A/B 实测已确认附加贴图不会产生材质效果。
