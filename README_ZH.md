...
目前支持 Minecraft 1.21.1 的 Fabric 环境（NeoForge 可通过 [Sinytra Connector](https://modrinth.com/mod/connector) 运行，渲染、持久化等核心功能已实测可用）。
...
- [ ] **LabPBR 材质**：此冻结的 flash 分支不支持 LabPBR 附加贴图。兼容 Iris 的光影加载器可能发现同名的 `_n.png`／`_s.png`，但 billboard 和 ring 图元使用 Minecraft 的基础位置／纹理／颜色 shader，不提供法线或实体材质采样器；`glowing: false` 只会应用环境亮度。游戏内 A/B 实测已确认附加贴图不会产生材质效果。
...
1. 为 Minecraft 1.21.1 安装 [Fabric Loader](https://fabricmc.net/use/) 0.16.0+。
...
编译好的 JAR 文件位于 `build/libs/halo-1.21.1-fabric-1.2.1.jar`。
...
- **开发环境**：Minecraft 1.21.1 + Fabric Loader 0.16.0+
