...
class YsmCompatTest {
    ...
    void localOffsetViewRestoreAndYawPitchRollSigns() {
        ...
        assertVector(new Vec3(frame.forward().x, frame.forward().y, frame.forward().z), actualForward);
        assertVector(new Vec3(frame.headUp().x, frame.headUp().y, frame.headUp().z), actualUp);
    }
    ...
    void absentOrUnreadableOptionalClassesDisableCapture() {
        org.junit.jupiter.api.Assertions.assertTrue(YsmAbiDetector.incompatibility(name -> null).isPresent());
        org.junit.jupiter.api.Assertions.assertTrue(YsmAbiDetector.incompatibility(name -> {
            throw new java.io.IOException("fixture read failure");
        }).orElseThrow().contains("fixture read failure"));
    }
}
