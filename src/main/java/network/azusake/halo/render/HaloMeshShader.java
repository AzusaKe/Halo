...
/** Immutable 26.2 pipeline state; per-draw material values belong to HaloDrawSubmitter. */
public final class HaloMeshShader {
    ...
    private static RenderPipeline pipeline(Key key) {
        ...
        if (!key.gui() && net.neoforged.fml.ModList.get().isLoaded("iris")) network.azusake.halo.compat.iris.IrisMeshBridge.assign(pipeline, key.lit(), key.blend());
        return pipeline;
    }
}
