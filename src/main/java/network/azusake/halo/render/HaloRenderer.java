...
public final class HaloRenderer {
    ...
    public void clearWorld(){
        previousWorld=null;submissions.clear();
        MeshRenderMetrics.resetFrame();
        meshBuffers.close();primitiveBuffers.close();HaloMeshShader.clear();
    }
    ...
    public void renderHalos(PoseStack matrices,Camera camera,float tickDelta){
        ...
        if(previousWorld!=client.level) {
            ...
            reloadMeshBuffers(HaloMeshResources.snapshot().visuals());
    ...
    private void submit(FrameOutput output,VisualResources visuals,Matrix4f outer,boolean shaderPack,RenderEnvironment environment,boolean late){
        long started = MeshRenderMetrics.start();
        ...
        if(environment == RenderEnvironment.WORLD) MeshRenderMetrics.finish(started, late);
    }
