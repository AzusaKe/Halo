...
public final class EmfVersionGate {
    ...
    public static boolean isSupportedVersion(String version) {
        ...
        int[] minimum = parseCore(Emf261Symbols.MIN_SUPPORTED_VERSION);
    ...
    public static Optional<String> installedVersion() {
        ...
        var modFile = loadingMods.getModFileById(Emf261Symbols.MOD_ID);
        if (modFile == null) {
            return Optional.empty();
        }
        return modFile.getMods().stream()
            .filter(mod -> Emf261Symbols.MOD_ID.equals(mod.getModId()))
